...
from .models import CreditUser, NoteUser, AdresseUser, ChoixRole, Voiture, TrajetProposer, Preference, ReservationTrajet ,  ChangerStatutTrajet
...
class UserAdmin(admin.ModelAdmin):
    list_display = ['id', 'username', 'email', 'password', 'first_name', 'last_name', 'is_active', 'is_staff', 'is_superuser', 'date_joined', 'last_login']
...
class NoteUserAdmin(admin.ModelAdmin):
    list_display = ['id', 'passager','chauffeur', 'note', 'commentaire', 'avis']
    search_fields = ['passager','chauffeur', 'note', 'commentaire', 'avis']
    list_filter = ['passager','chauffeur', 'note', 'commentaire', 'avis']
    list_per_page = 10

@admin.register(AdresseUser)
class AdresseUserAdmin(admin.ModelAdmin):
    list_display = ['id', 'user','numero', 'type_voie', 'complement', 'nom_rue', 'code_postal', 'ville', 'pays', 'telephone', 'email', 'photo']
    search_fields = ['user', 'numero', 'type_voie', 'complement', 'nom_rue', 'code_postal', 'ville', 'pays', 'telephone', 'email', 'photo']
    list_filter = ['user', 'numero', 'type_voie', 'complement', 'nom_rue', 'code_postal', 'ville', 'pays', 'telephone', 'email', 'photo']
    list_per_page = 104
    fieldsets = [
...
class VoitureAdmin(admin.ModelAdmin):
    fieldsets= [
        ...
        ('Caractéristiques', {'fields': ['couleur', 'type_moteur', 'places', 'immatriculation', 'annee']}),
            ]
    list_display = ['id', 'user', 'marque', 'modele', 'couleur', 'type_moteur', 'places', 'immatriculation', 'annee']
    search_fields = ['user', 'marque', 'modele', 'couleur', 'type_moteur', 'places', 'immatriculation', 'annee']
...
class TrajetProposerAdmin(admin.ModelAdmin):
    list_display = ['id', 'chauffeur','etat','trajet_rembourser','chauffeur','ville_depart','ville_arrivee','date','heure','type_moteur', 'prix', 'total_payer','temps_trajet','voiture' ]
    search_fields = ['chauffeur','etat','trajet_rembourser','chauffeur','ville_depart','ville_arrivee','date','heure','type_moteur', 'prix', 'total_payer','temps_trajet','voiture']
    list_filter = ['chauffeur','etat','trajet_rembourser','chauffeur','ville_depart','ville_arrivee','date','heure','type_moteur', 'prix', 'total_payer','temps_trajet','voiture']
    list_per_page = 10
    fieldsets = [
...
class PreferenceAdmin(admin.ModelAdmin):
    list_display =[ 'id', 'user_preference', 'exigences_particulieres', 'exigences_personnelles', 'fumeur','animaux']
    search_fields = ['user_preference', 'exigences_particulieres', 'exigences_personnelles', 'fumeur','animaux']
    list_filter = ['user_preference', 'exigences_particulieres', 'exigences_personnelles', 'fumeur','animaux']
    list_per_page = 10

@admin.register(ReservationTrajet)
class ReservationTrajetAdmin(admin.ModelAdmin):
    list_display =['id','trajet_reserver', 'passager','prix_par_passager','places', 'reservation_rembourser', 'etat_reservation' ]
    search_fields = ['trajet_reserver', 'passager','prix_par_passager','places', 'reservation_rembourser', 'etat_reservation']
    list_filter = ['trajet_reserver', 'passager','prix_par_passager','places', 'reservation_rembourser', 'etat_reservation']
    list_per_page = 10
    fieldsets = [
        ('Qui ?', {'fields': ['passager']}),
