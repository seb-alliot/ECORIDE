    ...
    if email_type in ["Avis positif", "Avis negatif"]:
        if title_id:
            ...
            try:
                ...
                chauffeur = getattr(trajet, "chauffeur", None)
                ...
                date_resa = trajet.date.strftime("%d/%m/%Y") if trajet else None
                info_trajet = {
                    "trajet": trajet,
                    "chauffeur": chauffeur,
                    "passager": passager,
                    "date reservation": date_resa,
                    "reservation": reservation
                }
                list_error = []
                for nom, valeur in info_trajet.items():
                    if not valeur:
                        list_error.append(nom)
